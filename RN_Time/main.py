...
parser.add_argument('--data_name', default='computers', type=str, help="data name")  # cora citeseer pubmed photo computers
...
parser.add_argument('--Mark_time', default=0, type=int)
parser.add_argument('--process_time', default=0, type=float)
...
def main(args):
    ...
    run_time = [[] for _ in range(args.run_split_num)]
    for iter_split_seed in range(args.run_split_num):
        ...
        target_data, feats, process_time_temp = load_processed_data(args, args.data_path, args.data_name,
                                          shuffle_seed=args.shuffle_seed_list[iter_split_seed],
                                          ppr_file=args.ppr_file)  # torch.where(target_data.rn_weight!=0)[0].shape->133
        setattr(args, 'num_feature', target_data.num_features)
        setattr(args, 'num_class', target_data.num_classes)

        if args.Mark_time == 0:
            args.process_time = process_time_temp
            args.Mark_time = 1
        ...
        for iter_init_seed in range(args.run_init_num):
            ...
            run_time[iter_split_seed].append(used_time)
    ...
    return val_macro_np, weighted_np, macro_np, time_np
    ...
    for lr in lrs:
        for wd in wds:
            for renode_reweight in rrs:
                ...
                val_macro_np, weighted_np, macro_np, run_time = main(args)
                print('\nThe overall performance:')
                ...
                time_mean = round(np.mean(run_time), 4)
                time_std = round(np.std(run_time), 4)

                temp = np.array([lr, wd, renode_reweight, val_macro_mean, val_macro_std,
                                 weighted_mean, weighted_std, macro_mean, macro_std, time_mean, time_std])
                results.append(temp)

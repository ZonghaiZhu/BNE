...
def get_step_split(args, all_idx, all_label, nclass):
    ...
    tail_list = list(set(all_class_list) - set(head_list))  # tail class labels [3,4,5,6,7]
...
def get_renode_weight(opt, data):
    ppr_matrix = data.Pi  # personlized pagerank, ->2708,2708
    gpr_matrix = torch.tensor(data.gpr).float()  # class-accumulated personlized pagerank, ->2708,7

    base_w = opt.rn_base_weight
    scale_w = opt.rn_scale_weight
    nnode = ppr_matrix.size(0)  # 2708
    unlabel_mask = data.train_mask.int().ne(1)  # unlabled node

    # computing the Totoro values for labeled nodes
    gpr_sum = torch.sum(gpr_matrix, dim=1)  # 2708
    gpr_rn = gpr_sum.unsqueeze(1) - gpr_matrix  # 2708,7
    rn_matrix = torch.mm(ppr_matrix, gpr_rn)

    label_matrix = F.one_hot(data.y, gpr_matrix.size(1)).float()
    label_matrix[unlabel_mask] = 0

    rn_matrix = torch.sum(rn_matrix * label_matrix, dim=1)  # 2708
...
def load_processed_data(args, data_path, data_name, shuffle_seed=0, ppr_file=''):
    ...
    random.shuffle(mask_list)  # 打乱样本顺序
    ...
    if args.size_imb_type == 'equal':
        ...
        # train_mask_list, valid_mask_list, test_mask_list, target_data.train_node = \
        #     get_split(args, mask_list, target_data.y.numpy(), nclass=target_data.num_classes)
    ...
    if os.path.exists(ppr_file):
        target_data.Pi = torch.load(ppr_file)  # ->2708,2708
    else:
        pr_prob = 1 - args.pagerank_prob
        A = index2dense(target_data.edge_index, target_data.num_nodes)  # A->2708,2708
        A_hat = A.to(args.device) + torch.eye(A.size(0)).to(args.device)  # add self-loop
        D = torch.diag(torch.sum(A_hat, 1))
        D = D.inverse().sqrt()
        A_hat = torch.mm(torch.mm(D, A_hat), D)
        target_data.Pi = pr_prob * (
            (torch.eye(A.size(0)).to(args.device) - (1 - pr_prob) * A_hat).inverse())  # 文中P,Node influence conflict
    ...
    for iter_c in range(target_data.num_classes):
        iter_Pi = target_data.Pi[torch.tensor(target_data.train_node[iter_c]).long()]  # 35,2708,每个训练样本对其他样本的作用
        iter_gpr = torch.mean(iter_Pi, dim=0).squeeze()  # ->2708
        gpr_matrix.append(iter_gpr)  # 训练样本每一类对其他所有样本的作用

    temp_gpr = torch.stack(gpr_matrix, dim=0)  # ->7,2708
    temp_gpr = temp_gpr.transpose(0, 1)  # ->2708,7
    target_data.gpr = temp_gpr

    target_data.rn_weight = get_renode_weight(args, target_data)  # ReNode Weight ->2708
    end_time = time.time() - start_time

    return target_data, torch.tensor(feats, dtype=torch.float32), end_time

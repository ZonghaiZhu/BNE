...
def load_processed_data(args, data_path, data_name, shuffle_seed=0):
    ...
    unlabeled_feats = []
    for i in range(num_classes):
        train_feats.append(feat[train_idx[i]])
        valid_feats.append(feat[val_idx[i]])
        test_feats.append(feat[test_idx[i]])
        #unlabeled_feats.append()

        train_labels.extend([i]*len(train_idx[i])) # must assign new label, (labels[explored_idx] leak labels)
    ...
    unlabeled_feats = torch.cat((valid_feats, test_feats), dim=0)

    std_s = torch.std(train_feats, dim=0)
    std_w = std_s/num_classes
    unlabeled_s = unlabeled_feats + np.random.multivariate_normal(mean=np.zeros((unlabeled_feats.shape[1])),
                                  cov=np.diag(std_s), size=unlabeled_feats.shape[0]).astype(np.float32)
    unlabeled_w = unlabeled_feats + np.random.multivariate_normal(mean=np.zeros((unlabeled_feats.shape[1])),
                                  cov=np.diag(std_w), size=unlabeled_feats.shape[0]).astype(np.float32)

    return train_feats, valid_feats, test_feats, \
           torch.tensor(train_labels), torch.tensor(valid_labels), torch.tensor(test_labels), unlabeled_s, unlabeled_w